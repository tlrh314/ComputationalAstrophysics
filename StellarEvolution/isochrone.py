...
"""
isochrone.py
Hacked version of example code obtained from amusecode.org
Original python scripts are test_HRdiagram_tracks.py
...
This script will generate HR diagram isochrones for
clusters with salpeter initial mass functions.
"""


def stellar_remnant_state(star):
    """
    Stellar type: desciption
    ...
    Returns True if star is still Main Sequence, else False.
    """
...
def simulate_evolution_tracks(
    ...
    which can be more easily accomplished by creating an array (stars = datamodel.Stars(number_of_stars))
