...
\begin{eqnarray}
    \frac{L_{\rm star}}{L\Sun} = \frac{(M_{V,\rm star}-M_{V,\odot})}{2.5} \left(\frac{d_{\rm star}}{d\Sun}\right)^2 \label{eq:lum},
\end{eqnarray}
...
Fig.~\ref{fig:bestfit} shows the observed dataset converted to luminosity and effective temperature in red and the numerical values in green. In this particular plot we show the best fit. Fig~\ref{fig:chisq} shows the REPLACE. In order to determine which fit is the best REPLACE WITH METHOD.
